        ...
        if ($end - $start >= 100 && $start - $time >=10000 ) {
            if ($record['uploaded_by'] == Auth::guard('teacher')->user()->name) {
                ...
                if ($rec->save()) {
                    ...
                    // Create a new file for that particular event with unique code
                    Storage::put('record/' . $record['code'] . '.txt', '');

                    return Redirect::to('create')->with(
                        [
            ...
            $errors = new MessageBag(
                [
                'uploaded_by' => ['This field must be your name']
                ]
            );
            return Redirect::back()->withErrors($errors)->withInput()->with(
                [
                'message' => 'Event must be created by you',
                'class' => 'Warning'
                ]
            );
        } else {
    ...
    public function openEvent($code)
    {
        ...
        if ($result) {
            if ($result['upload_id'] == Auth::guard('teacher')->user()['id']) {
                ...
                return view('program.update')->with(
                    'message',
                    [
                    $mess, $class, $timing
                    ]
